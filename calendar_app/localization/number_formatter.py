...
class NumberFormatter:
    ...
    def __init__(self):
        ...
        self.western_arabic_locales = {
            'bg_BG', 'ca_ES', 'cs_CZ', 'da_DK', 'de_DE', 'el_GR', 'es_ES', 'et_EE', 'fi_FI',
            'fr_CA', 'fr_FR', 'he_IL', 'hr_HR', 'hu_HU', 'id_ID', 'it_IT', 'ja_JP', 'ko_KR',
            'lt_LT', 'lv_LV', 'nb_NO', 'nl_NL', 'pl_PL', 'pt_BR', 'pt_PT', 'ro_RO', 'ru_RU',
            'sk_SK', 'sl_SI', 'sv_SE', 'tr_TR', 'uk_UA', 'vi_VN', 'zh_CN', 'zh_TW'
        }
    
    def format_number(self, number: int, locale: str = 'en_US', use_native: bool = True) -> str:
        """
        ...
        try:
            ...
            if use_native and locale in self.number_systems:
                ...
                formatted = ''.join(number_map.get(digit, digit) for digit in number_str)
                logger.debug(f"Formatted {number} for {locale}: {number_str} -> {formatted}")
                return formatted
        ...
        try:
            ...
            if locale in self.native_number_locales:
                ...
                if locale == 'ar_SA':
                    ...
                    # Hindi ordinals
                    return f"{formatted_number}वां" if number > 1 else f"{formatted_number}ला"
                elif locale == 'th_TH':
                    # Thai ordinals
            ...
            logger.warning(f"Failed to format ordinal {number} for locale {locale}: {e}")
            return str(number)
