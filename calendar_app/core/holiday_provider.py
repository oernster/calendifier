...
class UKHolidayProvider:
    ...
    def _get_fallback_holidays(self) -> Dict[str, Dict[int, List[tuple]]]:
        ...
        return {
            ...
            "Good Friday": {3: [(29, "Good Friday (approximate)")], 4: [(18, "Good Friday (approximate)")]},
            "Easter Monday": {3: [(1, "Easter Monday (approximate)")], 4: [(21, "Easter Monday (approximate)")]},
            "Early May Bank Holiday": {5: [(6, "Early May Bank Holiday (first Monday)")]},
            "Spring Bank Holiday": {5: [(27, "Spring Bank Holiday (last Monday)")], 6: [(3, "Spring Bank Holiday (last Monday)")]},
    ...
    def _get_fallback_holidays_for_year(self, year: int) -> Dict[date, Holiday]:
        ...
        for month, day, name in bank_holidays:
            try:
                ...
                if days_to_monday > 3:  # If more than 3 days away, go to previous Monday
                    days_to_monday -= 7
                
                actual_date = date.fromordinal(holiday_date.toordinal() + days_to_monday)
