...
class RRuleDialog:
    ...
    def __init__(self, parent: tk.Tk, i18n: I18nManager, initial_rrule: Optional[str] = None, 
    ...
    def _create_frequency_section(self, parent: ttk.Frame, row: int):
        ...
        frame = ttk.LabelFrame(parent, text=self.i18n.get("recurring.form.frequency"), padding="5")
        ...
        for i, (value, text) in enumerate(frequencies):
            rb = ttk.Radiobutton(frame, text=text, variable=self.frequency_var, 
                               value=value, command=self._on_frequency_change)
    ...
    def _create_interval_section(self, parent: ttk.Frame, row: int):
        ...
        frame = ttk.LabelFrame(parent, text=self.i18n.get("recurring.form.interval"), padding="5")
        frame.grid(row=row, column=0, sticky="ew", pady=(0, 5))
        frame.columnconfigure(2, weight=1)
        
        ttk.Label(frame, text=self.i18n.get("rrule.interval.every")).grid(row=0, column=0, padx=(0, 5))
        
        interval_spin = ttk.Spinbox(frame, from_=1, to=999, width=5, textvariable=self.interval_var,
                                  command=self._update_preview)
        interval_spin.grid(row=0, column=1, padx=(0, 5))
        interval_spin.bind('<KeyRelease>', lambda e: self._update_preview())
        
        self.interval_label = ttk.Label(frame, text=self.i18n.get("rrule.interval.weeks"))
    ...
    def _create_weekdays_section(self, parent: ttk.Frame, row: int):
        ...
        self.weekdays_frame = ttk.LabelFrame(parent, text=self.i18n.get("recurring.form.weekdays"), padding="5")
        ...
        for i, (day, text) in enumerate(weekdays):
            cb = ttk.Checkbutton(self.weekdays_frame, text=text, variable=self.weekday_vars[day],
                               command=self._update_preview)
            cb.grid(row=0, column=i, padx=(0, 10))
    
    def _create_monthly_section(self, parent: ttk.Frame, row: int):
        """Create monthly options section"""
        self.monthly_frame = ttk.LabelFrame(parent, text=self.i18n.get("recurring.advanced.by_monthday"), padding="5")
        self.monthly_frame.grid(row=row, column=0, sticky="ew", pady=(0, 5))
        self.monthly_frame.columnconfigure(1, weight=1)
        
        # By month day option
        ttk.Radiobutton(self.monthly_frame, text=self.i18n.get("recurring.advanced.by_monthday"),
                       variable=self.monthly_type_var, value="bymonthday",
                       command=self._update_preview).grid(row=0, column=0, sticky="w", pady=(0, 5))
        
        monthday_frame = ttk.Frame(self.monthly_frame)
        monthday_frame.grid(row=1, column=0, columnspan=2, sticky="ew", padx=(20, 0))
        
        ttk.Label(monthday_frame, text=self.i18n.get("recurring.form.monthday")).grid(row=0, column=0, padx=(0, 5))
        monthday_spin = ttk.Spinbox(monthday_frame, from_=1, to=31, width=5, textvariable=self.monthday_var,
                                  command=self._update_preview)
        monthday_spin.grid(row=0, column=1)
        monthday_spin.bind('<KeyRelease>', lambda e: self._update_preview())
        
        # By position option
        ttk.Radiobutton(self.monthly_frame, text=self.i18n.get("recurring.advanced.by_setpos"),
                       variable=self.monthly_type_var, value="bysetpos",
                       command=self._update_preview).grid(row=2, column=0, sticky="w", pady=(10, 5))
        ...
        setpos_combo = ttk.Combobox(setpos_frame, textvariable=self.setpos_var, values=[p[0] for p in positions],
        ...
        weekdays_combo = ttk.Combobox(setpos_frame, textvariable=self.setpos_weekday_var,
    ...
    def _create_end_condition_section(self, parent: ttk.Frame, row: int):
        ...
        frame = ttk.LabelFrame(parent, text=self.i18n.get("recurring.form.end_condition"), padding="5")
        frame.grid(row=row, column=0, sticky="ew", pady=(0, 5))
        frame.columnconfigure(1, weight=1)
        
        # Never end
        ttk.Radiobutton(frame, text=self.i18n.get("rrule.end.never"),
                       variable=self.end_type_var, value="never",
                       command=self._update_preview).grid(row=0, column=0, sticky="w", pady=(0, 5))
        
        # End after count
        ttk.Radiobutton(frame, text=self.i18n.get("rrule.end.after"),
                       variable=self.end_type_var, value="count",
                       command=self._update_preview).grid(row=1, column=0, sticky="w", pady=(0, 5))
        
        count_frame = ttk.Frame(frame)
        count_frame.grid(row=1, column=1, sticky="w", padx=(10, 0))
        
        count_spin = ttk.Spinbox(count_frame, from_=1, to=9999, width=8, textvariable=self.count_var,
                               command=self._update_preview)
        count_spin.grid(row=0, column=0, padx=(0, 5))
        count_spin.bind('<KeyRelease>', lambda e: self._update_preview())
        
        ttk.Label(count_frame, text=self.i18n.get("rrule.end.occurrences")).grid(row=0, column=1)
        
        # End until date
        ttk.Radiobutton(frame, text=self.i18n.get("rrule.end.until"),
                       variable=self.end_type_var, value="until",
                       command=self._update_preview).grid(row=2, column=0, sticky="w", pady=(0, 5))
    ...
    def _create_preview_section(self, parent: ttk.Frame, row: int):
        ...
        frame = ttk.LabelFrame(parent, text=self.i18n.get("recurring.preview"), padding="5")
    ...
    def _create_buttons_section(self, parent: ttk.Frame, row: int):
        ...
        for i, (preset, text) in enumerate(presets):
            btn = ttk.Button(preset_frame, text=text, command=lambda p=preset: self._apply_preset(p))
        ...
        ttk.Button(button_frame, text=self.i18n.get("dialog.cancel"), 
                  command=self._on_cancel).grid(row=0, column=0, padx=(0, 5))
        ttk.Button(button_frame, text=self.i18n.get("dialog.ok"), 
                  command=self._on_ok).grid(row=0, column=1)
    ...
    def _build_rrule(self) -> str:
        ...
        try:
            ...
            if self.end_type_var.get() == "count":
                ...
                try:
                    until_date = datetime.strptime(self.until_var.get(), "%Y-%m-%d").date()
    ...
    def _update_preview(self):
        ...
        try:
            ...
            if len(occurrences) > 20:
                self.preview_listbox.insert(tk.END, f"... and {len(occurrences) - 20} more")
            
        except Exception as e:
            logger.error(f"❌ Preview update failed: {e}")
            self.preview_text.set(f"{self.i18n.get('recurring.error.invalid_rrule')}: {e}")
            self.preview_listbox.delete(0, tk.END)
...
def show_rrule_dialog(parent: tk.Tk, i18n: I18nManager, initial_rrule: Optional[str] = None, 
