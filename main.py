...
os.environ['QT_LOGGING_RULES'] = 'qt.text.font.db=false;qt.text.font.db.debug=false;qt.text.font.db.warning=false'
...
from calendar_app.localization import I18nManager, get_text as _, set_locale, LocaleDetector
    ...
    def _set_application_icon(self):
        ...
        try:
            ...
            for size in sizes:
                ...
                font.setPixelSize(int(size * 0.8))  # Slightly smaller than the icon size
    ...
    def _sync_ntp_safe(self):
        ...
        if self.ntp_client:
            try:
                ...
                if result.success:
                    logging.debug(f"✅ NTP sync successful: {result.server} (offset: {result.offset:.3f}s)")
                else:
...
def main():
    ...
    try:
        ...
        logging.info(f"{UI_EMOJIS['app_icon']} Application exited with code {exit_code}")
        return exit_code
