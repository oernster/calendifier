#!/usr/bin/env python3
"""
Script to create a drag-to-install DMG installer for the Calendifier app.
This creates a user-friendly DMG with a background image that instructs
users to drag the app to the Applications folder, and applies the Calendifier
...
class ProcessTerminator:
    ...
    @staticmethod
    def terminate_running_instances(app_name: str = "Calendifier") -> bool:
        """
...
class BackgroundImageCreator:
    ...
    def create_background_image() -> bool:
        ...
        try:
            ...
            instruction1 = "To install Calendifier, drag it to the Applications folder."
            ...
            try:
                with open("dmg_background.txt", "w") as f:
                    ...
                    To install Calendifier, drag it to the Applications folder.
    ...
    def create_launcher_script(
        ...
        try:
            ...
            with open(launcher_path, "w") as f:
                f.write(
                    f"""#!/bin/bash
# Calendifier launcher script that uses Python inside the app bundle
...
LOG_FILE="/tmp/calendifier_launch.log"
exec 3>&1 4>&2
...
# Log system info
echo "==============================================="
echo "Calendifier Launcher - $(date)"
echo "==============================================="
...
if [[ "$APP_ROOT" == *"/Volumes/"* ]]; then
    ...
    tell application "Finder"
        activate
        display dialog "Please drag Calendifier to your Applications folder before running it." buttons {{"OK"}} default button 1 with icon stop with title "Calendifier"
    end tell
EOF
    exit 1
fi

# First try to use the bundled Python
BUNDLED_PYTHON="$PYTHON_BIN_DIR/python3"
if [ -x "$BUNDLED_PYTHON" ]; then
    echo "Using bundled Python: $BUNDLED_PYTHON"
    PYTHON_EXEC="$BUNDLED_PYTHON"
else
    # Fall back to system Python if bundled Python is not available
    ...
    # If we still don't have Python, show error and exit
    if [ -z "$SYSTEM_PYTHON" ]; then
        echo "No Python 3 found on the system"
        osascript <<EOF
        tell application "Finder"
            activate
            display dialog "Error: Calendifier requires Python 3, but no Python 3 was found on your system. Please install Python 3 from python.org and try again." buttons {{"OK"}} default button 1 with icon stop with title "Calendifier Error"
        end tell
EOF
        exit 1
    fi
...
# Check required packages beexecuting the main script
REQUIRED_PACKAGES=("PySide6" "dateutil" "holidays")
MISSING_PACKAGES=()
...
# Check for recurring event specific modules
echo "Checking for recurring event modules..."
if ! "$PYTHON_EXEC" -c "
import sys
sys.path.insert(0, '$RESOURCES_DIR')
from calendar_app.core.rrule_parser import RRuleParser
from calendar_app.core.recurring_event_generator import RecurringEventGenerator
print('Recurring event modules loaded successfully')
" 2>/dev/null; then
    echo "ERROR: Recurring event modules not found or not importable"
    MISSING_PACKAGES+=("recurring_modules")
else
    echo "✅ Recurring event modules found and importable"
fi
...
if [ "${{#MISSING_PACKAGES[@]}}" -gt 0 ]; then
    ...
    tell application "Finder"
        activate
        display dialog "Error: Calendifier requires the following Python packages: $MISSING_LIST. The application may be damaged or incompletely installed." buttons {{"OK"}} default button 1 with icon stop with title "Calendifier Error"
    end tell
EOF
    exit 1
fi
...
if [ ! -f "$MAIN_SCRIPT" ]; then
    ...
    tell application "Finder"
        activate
        display dialog "Error: Main application script not found. The application may be corrupted." buttons {{"OK"}} default button 1 with icon stop with title "Stellody Error"
    end tell
EOF
    exit 1
fi
...
# Run the application with our Python
echo "Launching application with command: $PYTHON_EXEC $MAIN_SCRIPT $@"
...
if [ $EXIT_CODE -ne 0 ]; then
    ...
    tell application "Finder"
        activate
        display dialog "Calendifier encountered an error. See log file at $LOG_FILE for details." buttons {{"OK", "Show Log"}} default button 1 with icon stop with title "Calendifier Error"
...
class IconConverter:
    ...
    def convert_png_to_icns(source_path: Path, app_bundle_path: Path) -> bool:
        ...
        try:
            ...
            icon_path = source_path / "assets" / "calendar_icon.png"
            if not os.path.exists(icon_path):
                print(f"Error: calendar_icon.png not found at {icon_path}")
                return False
            ...
            success, _ = CommandRunner.run_command(
                [
                    ...
                    str(icon_dir),
                    "-o",
                    str(resources_path / "calendifier.icns"),
                ]
            )
            if not success:
...
class PackageManager:
    ...
    def copy_packages(resources_path: Path, site_packages_path: Path) -> bool:
        ...
        try:
            ...
            # Special handling for recurring event critical dependencies
            try:
                print("Ensuring recurring event dependencies...")
                critical_packages = ["dateutil", "holidays"]
                for pkg_name in critical_packages:
                    pkg_path = os.path.join(site_packages_path, pkg_name)
...
<dict>
    ...
    <string>calendifier.icns</string>
    <key>CFBundleIdentifier</key>
    <string>com.calendifier.app</string>
    ...
    def copy_app_files(self) -> bool:
        ...
        try:
            ...
            # Copy entire calendar_app directory with all JSON localization data
            ...
            # Create accessible config.json if needed
...
class DMGCreator:
    ...
    def create(self) -> bool:
        ...
        try:
            ...
            # Create DMG with professional styling
            ...
            if result.returncode != 0:
                ...
                if fallback_result.returncode != 0:
                    print(f"Fallback method failed: {fallback_result.stderr}")
                    return False
